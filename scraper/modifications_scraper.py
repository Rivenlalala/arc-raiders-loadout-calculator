    ...
    for table in content.find_all('table'):
        for link in table.find_all('a'):
            ...
            if (href and
                ...
                not any(x in href for x in [':', 'Category', 'File', 'Special', 'Weapon_Modifications'])):
...
def extract_mod_data(url, mod_name):
    ...
    if infobox:
        # Get slot type from category tags
        for row in infobox.find_all('tr', {'class': 'data-tag'}):
            text = row.get_text(strip=True)
            if any(slot in text for slot in ['Muzzle', 'Underbarrel', 'Stock', 'Mag', 'Tech']):
                mod_data['slot_type'] = text
